...
/***/ function(module, exports, __webpack_require__) {
	...
	var _exercises = __webpack_require__(169);
...
/***/ function(module, exports) {
	...
	function drawChessboard(dimension) {
	  ...
	  function constructBoard(length) {
	    for (var i = length; i > 0; i--) {
	      ...
	      if (length < 1) return row + '\n';else return rowBuilder(tiles, !state, length - 1, state ? row + tiles.black : row + tiles.white);
	    }
	  }
	...
	console.log('test');
	// helper functions
	var constructHashRow = function constructHashRow(width, isCheckered, isOdd) {

	  var black = '#';
	  var white = ' ';

	  var buildRow = function buildRow(block, currentWidth, isCheckered) {
	    if (currentWidth === width) return;
	    if (!isCheckered) return block + buildRow(block, currentWidth - 1);
	    if (block === ' ') return block + buildRow(black, currentWidth - 1);else return block + buildRow(white, currentWidth - 1);
	  };

	  if (isCheckered) return isOdd ? buildRow(black, width, isCheckered) : buildRow(white, width, isCheckered);else return buildRow(black, width);
	};
